...
function start() {
  ...
  bees = new Array();
  lastStingTime = new Date(); //variable lastStingTime is initialised
  makeBees();
  updateBees();
}
...
function setSpeed() {
  bear.dBear = Number(document.getElementById("spBear").value); //Updates the speed of the bear according to the input value given by the user
}
...
function getRandomInt(max) {
  return Math.floor(Math.random() * max); //generates a random integer between 0 and max using math.floor and math.random functions
}
...
function updateBees() {
  ...
  let period = document.getElementById("periodTimer").value; //gets the input from the user and updates the value
